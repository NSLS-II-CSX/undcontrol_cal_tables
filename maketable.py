...
def load_input_pd_csv(file, auto_xy = True):
    '''file : csv file created by bandas to_csv with index = False
        auto_xy : returns x, y tuple if True, else a DataFrame is return

    returns 
         data : x,y tuple or DataFrame (for more manual selection)
    '''   
    ...
    if auto_xy is False:
        ...
        xe = df.iloc[:,0].values#makes array
        yg = df.iloc[:,1].values
        return xe, yg
...
def fit_input_make_output(xe, yg, Epts = 1000, Emin = None, Emax = None, upload_gap_um=True, polydeg = 7, file=None, one2three=False):
    ...
    if upload_gap_um:
        output = np.array([xnew, ynew*1000])
    else:
...
def set_EPUioc_table(file, csv_pandas=True, Epts=1000, ioc_table_num=None, write_to_ioc=False, 
                     Emin = None, Emax = None, upload_gap_um=True, third_fm_first = False, polydeg = 7 ):
    ...
    upload_gap_um\t :   \tTrue to convert mm to um and upload that to IOC.  False retains mm. (CSX is really um, SIX is mm)
    ...
    output = fit_input_make_output(xe, yg, Epts, Emin, Emax, upload_gap_um, polydeg, file, third_fm_first)
